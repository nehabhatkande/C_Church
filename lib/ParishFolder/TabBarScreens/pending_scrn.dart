...
class pending_scrn extends StatefulWidget {
  const pending_scrn({super.key});

  @override
  State<pending_scrn> createState() => _pending_scrnState();
}

class _pending_scrnState extends State<pending_scrn> {
  @override
  void initState() {
  ...
  // ============
  List<pending_models> pm = [];
  pending_Async() async {
    pm = [];
    var tlvStr =
        " select b_id,user_id,booktype,prayertype,pr_date,slot,status from prayer.booking where ch_id = ${glb.church.ch_id} and status = '0';";
